...
function start() {
	...
	var categoryLines = window.lineSvg.selectAll("#Lines > g");
	...
	for(var i = 0; i < categoryLines.length; i++) {
		...
		line.mouseover(function(e) {
			var selectedCatId = parseInt(e.target.parentNode.id.split("-")[1]);
		...
		line.mouseout(function(e) {
			var selectedCatId = e.target.parentNode.id.split("-")[1];
			for(var j = 0; j < categoryLines.length; j++) {
