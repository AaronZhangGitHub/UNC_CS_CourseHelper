...
function start() {
	...
	var categoryLines = window.mysvgobj.selectAll("#Lines g");
	var classNodes = window.mysvgobj.selectAll("#Text g");

	for(var i = 0; i < categoryLines.length; i++) {
		var line = categoryLines[i];
		line.hover(function(e) {
			var line = window.mysvgobj.select("#" + e.target.parentNode.id);
			console.log(e);
			line.attr({opacity: .1});
		});
